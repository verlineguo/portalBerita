...
@section('content')
    <div class="page-content">
        ...
        <h5 class="mb-0 text-uppercase">Comment</h5>
        <hr />
                            ...
                            @foreach ($contacts as $contact)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('admin.contact.show', $contact->id) }}" class="text-primary"
                                            style="font-size: 24px;">
                                            <i class="bx bxs-show"></i>
                                        <a href="{{ route('admin.contact.delete', $contact->id) }}" class="ms-1 text-danger"
                                            style="font-size: 24px;"
                                            onclick="event.preventDefault(); confirmDelete('{{ $contact->id }}')">
                                            <i class="bx bxs-trash"></i>
                                        </a>

                                        <form id="delete-form-{{ $contact->id }}"
                                            action="{{ route('admin.contact.delete', $contact->id) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
